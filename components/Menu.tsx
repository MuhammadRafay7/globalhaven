...
const List: FC<ListProps> = ({ children, className, position = "bottom-right" }) => {
  ...
  useKeyPress({ key: "Escape", action: close, enable: !!openId });
    ...
    <AnimatePresence>
      {!openId ? null : (
        <motion.ul
          ...
          style={{ originY: 0 }}
          className={cn(
            "absolute top-[110%] w-max min-w-[190px] rounded-xl z-[9999] text-[13px] overflow-hidden border",
  ...
  return (
    <li className="w-full">
      <button
        ...
        type="button"
        className={cn(
          "text-left px-4 py-3 w-full text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-dark-border/50 transition-colors duration-100",
          className
        )}
      >
        {children}
